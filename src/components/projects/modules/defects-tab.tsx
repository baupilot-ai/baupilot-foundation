  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-wrap gap-2">
          <Select value={statusF} onValueChange={setStatusF}>
            ...
            <SelectContent>
              ...
              {DEFECT_STATUS.map((s) => <SelectItem key={s.value} value={s.value}>{s.label}</SelectItem>)}
            </SelectContent>
          </Select>
        ...
        <Button onClick={() => { setEditing(null); setOpen(true); }}><Plus className="h-4 w-4" /> New defect</Button>
      </div>
      ...
      {loading ? (
        ...
        <Card className="border-dashed border-border/70 bg-muted/30">
          <CardContent className="flex flex-col items-center gap-3 px-6 py-16 text-center">
            ...
            <p className="text-sm font-medium">No defects logged</p>
            <Button onClick={() => { setEditing(null); setOpen(true); }}><Plus className="h-4 w-4" />Log defect</Button>
          </CardContent>
        </Card>
      ) : (
                  ...
                  <div className="flex gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-2 flex flex-wrap items-center gap-2">
                        ...
                        {d.due_date && <span className="inline-flex items-center gap-1 text-xs text-muted-foreground"><Calendar className="h-3 w-3" />{new Date(d.due_date).toLocaleDateString()}</span>}
                      </div>
                    </div>
                    <div className="flex flex-col gap-1">
                      <Button size="icon" variant="ghost" onClick={() => { setEditing(d); setOpen(true); }}><Edit className="h-4 w-4" /></Button>
                      <Button size="icon" variant="ghost" onClick={() => setConfirmDel(d)}><Trash2 className="h-4 w-4 text-destructive" /></Button>
                    </div>
                  </div>
                </CardContent>
              </Card>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={async () => {
                ...
                try { await deleteDefect(confirmDel.id); toast.success("Deleted"); setConfirmDel(null); load(); }
                catch (e) { toast.error(e instanceof Error ? e.message : "Failed"); }
              }}
...
function DefectDialog({
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (editing) { await updateDefect(editing.id, payload, { project_id: projectId }); toast.success("Updated"); }
      else { await createDefect({ ...payload, project_id: projectId, title: form.title } as never); toast.success("Created"); }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
        <DialogHeader><DialogTitle>{editing ? "Edit defect" : "New defect"}</DialogTitle></DialogHeader>
        <form onSubmit={onSubmit} className="space-y-3">
          ...
          <div className="space-y-1.5"><Label className="text-xs">Location</Label><Input value={form.location ?? ""} onChange={(e) => setForm({ ...form, location: e.target.value })} placeholder="Floor 2 / Room 204" /></div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="space-y-1.5"><Label className="text-xs">Status</Label>
              <Select value={form.status ?? "open"} onValueChange={(v) => setForm({ ...form, status: v })}>
                ...
                <SelectContent>{DEFECT_STATUS.map((s) => <SelectItem key={s.value} value={s.value}>{s.label}</SelectItem>)}</SelectContent>
              </Select>
            </div>
            ...
            <div className="space-y-1.5"><Label className="text-xs">Responsible</Label><Input value={form.responsible_person ?? ""} onChange={(e) => setForm({ ...form, responsible_person: e.target.value })} /></div>
