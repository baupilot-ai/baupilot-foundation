  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => setOpen(true)}><Plus className="h-4 w-4" />Add record</Button>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid gap-3">
          {items.map((m) => {
            ...
            const tl = tools.find((t) => t.id === m.tool_id);
            return (
              <Card key={m.id} className="border-border/70">
                <CardContent className="p-4 space-y-1">
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {eq ? `Equipment: ${eq.name}` : tl ? `Tool: ${tl.name}` : "—"} ·{" "}
                        {m.record_type} · {m.performed_date ? new Date(m.performed_date).toLocaleDateString() : "—"}
                      </p>
                    </div>
                    <div className="flex gap-2 items-center">
                      <StatusBadge tone={m.status === "completed" ? "success" : "info"}>{m.status}</StatusBadge>
                      <Button size="icon" variant="ghost" onClick={async () => {
                        try { await deleteMaintenance(m.id); toast.success("Deleted"); load(); }
                        catch (e) { toast.error(e instanceof Error ? e.message : "Failed"); }
                  ...
                  <div className="text-xs text-muted-foreground">
                    ...
                    {m.next_due_date && <>Next due {new Date(m.next_due_date).toLocaleDateString()}</>}
                  </div>
                </CardContent>
              </Card>
            );
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-xl">
        ...
        <form className="space-y-3" onSubmit={async (e) => {
          ...
          if (target === "equipment" && !form.equipment_id) { toast.error("Select equipment"); return; }
          if (target === "tool" && !form.tool_id) { toast.error("Select tool"); return; }
          setSaving(true);
          try {
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <Field label={target === "equipment" ? "Equipment" : "Tool"}>
              {target === "equipment" ? (
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <Field label="Performed date"><Input type="date" value={form.performed_date ?? ""} onChange={(e) => setForm((f) => ({ ...f, performed_date: e.target.value }))} /></Field>
            <Field label="Next due"><Input type="date" value={form.next_due_date ?? ""} onChange={(e) => setForm((f) => ({ ...f, next_due_date: e.target.value }))} /></Field>
            <Field label="Performed by"><Input value={form.performed_by ?? ""} onChange={(e) => setForm((f) => ({ ...f, performed_by: e.target.value }))} /></Field>
            <Field label="Cost (€)"><Input type="number" step="0.01" value={form.cost ?? ""} onChange={(e) => setForm((f) => ({ ...f, cost: e.target.value ? Number(e.target.value) : undefined }))} /></Field>
          </div>
