...
function ProjectDetail() {
    ...
    return (
      <div className="space-y-4">
        <Button variant="ghost" size="sm" asChild>
          <Link to="/projects"><ArrowLeft className="h-4 w-4" />{t("common.back")}</Link>
        </Button>
  ...
  async function onArchive() {
    ...
    toast.success(project.archived_at ? t("projects.restored") : t("projects.archived"));
    await load();
  }
  ...
  return (
    <div className="space-y-6">
      <Button variant="ghost" size="sm" asChild className="-ml-2 w-fit">
        <Link to="/projects"><ArrowLeft className="h-4 w-4" />{t("projects.backToProjects")}</Link>
      </Button>
      ...
      <Tabs defaultValue="overview" className="space-y-4">
        <div className="overflow-x-auto">
          <TabsList className="w-max">
            ...
            <TabsTrigger value="daily"><CalendarDays className="h-4 w-4" />{t("projects.tabs.daily")}</TabsTrigger>
            <TabsTrigger value="tasks"><CheckSquare className="h-4 w-4" />{t("projects.tabs.tasks")}</TabsTrigger>
            <TabsTrigger value="defects"><AlertOctagon className="h-4 w-4" />{t("projects.tabs.defects")}</TabsTrigger>
            <TabsTrigger value="photos"><Camera className="h-4 w-4" />{t("projects.tabs.photos")}</TabsTrigger>
            <TabsTrigger value="documents"><FileText className="h-4 w-4" />{t("projects.tabs.documents")}</TabsTrigger>
            <TabsTrigger value="plans"><Layers className="h-4 w-4" />{t("projects.tabs.plans")}</TabsTrigger>
            <TabsTrigger value="team"><Users className="h-4 w-4" />{t("projects.tabs.team")}</TabsTrigger>
            <TabsTrigger value="contacts"><UserSquare className="h-4 w-4" />{t("projects.tabs.contacts")}</TabsTrigger>
            <TabsTrigger value="equipment"><Hammer className="h-4 w-4" />{t("projects.tabs.equipment")}</TabsTrigger>
        ...
        <TabsContent value="overview" className="space-y-4">
          ...
          <div className="grid gap-4 lg:grid-cols-3">
            <Card className="border-border/70 lg:col-span-2">
              <CardHeader><CardTitle className="text-base">{t("projects.detail.projectInformation")}</CardTitle></CardHeader>
              <CardContent className="space-y-4">
                {project.description && <p className="text-sm leading-relaxed">{project.description}</p>}
                <dl className="grid gap-x-6 gap-y-3 sm:grid-cols-2">
                  <Detail icon={MapPin} label={t("projects.fields.siteAddress")} value={project.site_address} />
                  <Detail label={t("projects.fields.gps")} value={project.gps_lat != null && project.gps_lng != null ? `${project.gps_lat}, ${project.gps_lng}` : null} />
                  <Detail label={t("projects.fields.type")} value={project.project_type ? t(`projects.types.${project.project_type}`, { defaultValue: project.project_type }) : null} />
                  <Detail label={t("projects.fields.category")} value={project.building_category ? t(`projects.categories.${project.building_category}`, { defaultValue: project.building_category }) : null} />
                  <Detail label={t("projects.fields.phase")} value={project.construction_phase ? t(`projects.phases.${project.construction_phase}`, { defaultValue: project.construction_phase }) : null} />
                  <Detail label={t("projects.fields.status")} value={t(`projects.status.${project.current_status}`, { defaultValue: project.current_status })} />
                </dl>
              </CardContent>
            </Card>
            <Card className="border-border/70">
              <CardHeader><CardTitle className="text-base">{t("projects.detail.keyDates")}</CardTitle></CardHeader>
              <CardContent>
                <dl className="space-y-3">
                  <Detail icon={Calendar} label={t("projects.fields.plannedStart")} value={formatDate(project.planned_start)} />
                  <Detail icon={Calendar} label={t("projects.fields.plannedFinish")} value={formatDate(project.planned_finish)} />
                  <Detail icon={Calendar} label={t("projects.fields.actualStart")} value={formatDate(project.actual_start)} />
                  <Detail icon={Calendar} label={t("projects.fields.actualFinish")} value={formatDate(project.actual_finish)} />
                </dl>
              </CardContent>
            </Card>
            {project.notes && (
              <Card className="border-border/70 lg:col-span-3">
                <CardHeader><CardTitle className="text-base">{t("projects.detail.notes")}</CardTitle></CardHeader>
                <CardContent>
        ...
        <TabsContent value="notifications"><NotificationsTab projectId={project.id} /></TabsContent>



        <TabsContent value="timeline" className="space-y-4">
          <Card className="border-border/70">
            <CardHeader><CardTitle className="text-base">{t("projects.detail.schedule")}</CardTitle></CardHeader>
            <CardContent className="space-y-4">
              <dl className="grid gap-x-6 gap-y-3 sm:grid-cols-2">
                <Detail icon={Calendar} label={t("projects.fields.plannedStart")} value={formatDate(project.planned_start)} />
                <Detail icon={Calendar} label={t("projects.fields.plannedFinish")} value={formatDate(project.planned_finish)} />
        ...
        <TabsContent value="financials" className="space-y-4">
          <Card className="border-border/70">
            <CardHeader><CardTitle className="text-base">{t("projects.detail.contract")}</CardTitle></CardHeader>
            <CardContent>
              <Detail
                icon={Euro}
                label={t("projects.detail.contractValue")}
                value={project.contract_value != null ? formatCurrency(project.contract_value) : null}
              />
            </CardContent>
          </Card>
          <div className="grid gap-4 sm:grid-cols-3">
            {[
              { title: t("projects.detail.costs"), desc: t("projects.detail.costsDesc") },
              { title: t("projects.detail.invoices"), desc: t("projects.detail.invoicesDesc") },
              { title: t("projects.detail.changeOrders"), desc: t("projects.detail.changeOrdersDesc") },
            ].map((c) => (
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {t("projects.deleteConfirmDescShort", { name: project.name })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
function TimelineSummary({ project }: { project: ProjectRow }) {
  ...
  let indicator: { tone: "info" | "success" | "warning" | "neutral"; label: string } = {
    tone: "neutral",
    label: t("projects.detail.noSchedule"),
  };
  if (project.current_status === "completed") indicator = { tone: "success", label: t("projects.detail.completed") };
  else if (daysRemaining != null) {
    if (daysRemaining < 0) indicator = { tone: "warning", label: t("projects.detail.daysOverdue", { count: Math.abs(daysRemaining) }) };
    else if (daysRemaining <= 14) indicator = { tone: "warning", label: t("projects.detail.daysRemaining", { count: daysRemaining }) };
    else indicator = { tone: "info", label: t("projects.detail.daysRemaining", { count: daysRemaining }) };
  }
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 rounded-lg border border-border/70 bg-muted/40 p-4 text-sm">
      ...
      {totalDays != null && (
        <span className="text-muted-foreground">{t("projects.detail.totalDuration", { count: totalDays })}</span>
      )}
    </div>
  );
}
